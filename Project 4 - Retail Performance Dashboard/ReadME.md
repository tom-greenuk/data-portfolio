...
### Data Model
- `fact_sales` (many) → `store_detail` (one) via `store_no`
- Week-based time axis using `fact_sales[week]` — no date table required
- Organic classification handled via `store_detail` attributes
